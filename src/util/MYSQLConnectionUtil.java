...
public class MYSQLConnectionUtil {
	//mysql  JDBC(자바 데이터베이스 커넥션)연결하는 메소드 작성. - 리턴 타입 Connection
	...
	//DriverManager 클래스의 메소드 getConnection 으로 데이터베이스 드라이버에 대한 Connection 구현객체  생성
													//	    ㄴ인터페이스 Connection 타입으로 참조합니다.	
	...
	public static void close(Connection conn) {
		if(conn != null)  //conn 이 널이 아닐때만 close 실행하도록 합니다.
			try {
				...
				System.out.println("데이터베이스 연결 종료애 뮨제가 생겼습니다. : " + e.getMessage());
			}
	}
